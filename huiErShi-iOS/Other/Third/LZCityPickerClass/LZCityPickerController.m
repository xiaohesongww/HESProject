...
- (void)viewDidLoad {
    ...
    _cityPicker = [LZCityPickerView showInView:self.view didSelectWithBlock:^(NSString *address, NSString *province, NSString *city) {
        
        if (ws.backBlock) {
            ws.backBlock(address, province, city);
        }
    } cancelBlock:^{
