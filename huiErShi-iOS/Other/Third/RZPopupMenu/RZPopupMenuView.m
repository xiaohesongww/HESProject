...
@property (nonatomic, assign) CGFloat kRZPopupMenuCenterBtnCenterX;
@property (nonatomic, assign) CGFloat kRZPopupMenuCenterBtnCenterY;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.kRZPopupMenuCenterBtnCenterX = kSCREEN_WIDTH/2;
        self.kRZPopupMenuCenterBtnCenterY = kSCREEN_HEIGHT - 28;
        self.startPoint = CGPointMake(_kRZPopupMenuCenterBtnCenterX, _kRZPopupMenuCenterBtnCenterY);
...
- (void)setupMenuItems
{
    ...
    for (RZPopupMenuItem *item in self.menuItems) {
        ...
        item.startPoint = CGPointMake(_kRZPopupMenuCenterBtnCenterX, _kRZPopupMenuCenterBtnCenterY);
        
        CGPoint farPoint = CGPointMake(_kRZPopupMenuCenterBtnCenterX + kRZPopupMenuItemFarRadius * cosf(angle), _kRZPopupMenuCenterBtnCenterY + kRZPopupMenuItemFarRadius * sinf(angle));
        item.farPoint = RotateCGPointAroundCenter(farPoint, self.startPoint, kRZPopupMenuItemStartAngle);
        item.farPoint = farPoint;
        
         CGPoint nearPoint = CGPointMake(_kRZPopupMenuCenterBtnCenterX + kRZPopupMenuItemNearRadius * cosf(angle), _kRZPopupMenuCenterBtnCenterY + kRZPopupMenuItemNearRadius * sinf(angle));
        item.nearPoint = RotateCGPointAroundCenter(nearPoint, self.startPoint, kRZPopupMenuItemStartAngle);
        item.nearPoint = nearPoint;
        
        CGPoint endPoint = CGPointMake(_kRZPopupMenuCenterBtnCenterX + kRZPopupMenuItemEndRadius * cosf(angle), _kRZPopupMenuCenterBtnCenterY + kRZPopupMenuItemEndRadius * sinf(angle));
...
- (void)expandWithTag:(NSInteger)tag
{
    ...
    CGPathMoveToPoint(path, NULL, _kRZPopupMenuCenterBtnCenterX, _kRZPopupMenuCenterBtnCenterY);
...
- (UIButton *)centerBtn
{
    if (!_centerBtn) {
        ...
        [_centerBtn setImage:GETIMG(@"添加按钮") forState:UIControlStateNormal];
        [_centerBtn addTarget:self action:@selector(ControlItems:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _centerBtn;
}

//控制items
- (void)ControlItems:(UIButton *)sender {
...
- (NSMutableArray *)menuItems
{
    if (!_menuItems) {
        ...
        for (int i = 0; i < 4; i ++) {
            switch (i) {
                case 0:{
                    RZPopupMenuItem *item = [[RZPopupMenuItem alloc] initWithContentImage:GETIMG(@"饮食") Title:@"饮食"];
                    item.delegate = self;
                ...
                case 1:{
                    RZPopupMenuItem *item = [[RZPopupMenuItem alloc] initWithContentImage:GETIMG(@"运动") Title:@"运动"];
                    item.delegate = self;
                    item.tag = _menuItems.count;
                    [_menuItems addObject:item];
                    break;
                ...
                case 2:{
                    RZPopupMenuItem *item = [[RZPopupMenuItem alloc] initWithContentImage:GETIMG(@"自测") Title:@"自测"];
                    item.delegate = self;
                    item.tag = _menuItems.count;
                    [_menuItems addObject:item];
                    break;
                ...
                case 3:{
                    RZPopupMenuItem *item = [[RZPopupMenuItem alloc] initWithContentImage:GETIMG(@"用药") Title:@"用药"];
                    item.delegate = self;
                    item.tag = _menuItems.count;
                    [_menuItems addObject:item];
                    break;
