...
{
    [super awakeFromNib];
    [_collectionView registerNib:[UINib nibWithNibName:@"SZCalendarCell" bundle:[NSBundle mainBundle]] forCellWithReuseIdentifier:SZCalendarCellIdentifier];
...
- (void)setDate:(NSDate *)date
{
    ...
    NSInteger i = [self weekDay:date];
    [_monthLabel setText:[NSString stringWithFormat:@"%li年%.2ld月%.2ld日 星期%@",(long)[self year:date],(long)[self month:date],[self day:date],weekDayArray[i]]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.activityState.hidden = YES;
    cell.dateLabel.backgroundColor = [UIColor whiteColor];
    cell.progressBack.hidden = YES;
    if (indexPath.section == 0) {
        ...
        if (i < firstWeekday) {
            ...
            if (indexPath.row == 12||indexPath.row == 20) {
                cell.activityState.hidden = NO;
                cell.progressBack.hidden = NO;
            }
            //this month
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.calendarBlock) {
        self.calendarBlock( [comp weekday],day, [comp month], [comp year]);
    }
    if (indexPath.section !=0) {
